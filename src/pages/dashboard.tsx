...
import { getAddresses } from 'apiSdk/addresses';
import { AddressInterface } from 'interfaces/address';
import { getPhones } from 'apiSdk/phones';
import { PhoneInterface } from 'interfaces/phone';

import { PhoneListPage as ShowCaseList } from 'pages/phones';
...
function DashboardPage() {
  ...
  const addressStat = useSWR<PaginatedInterface<AddressInterface>>(
    () => `/addresses`,
    () => getAddresses({ limit: 1, offset: 0 }),
  );

  const phoneStat = useSWR<PaginatedInterface<PhoneInterface>>(
    () => `/phones`,
    () => getPhones({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Addresses',
      result: addressStat?.data?.totalCount,
      isLoading: addressStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Phones',
      result: phoneStat?.data?.totalCount,
      isLoading: phoneStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`phones`} passHref legacyBehavior>
                  <Button
