...
import { getAddresses, deleteAddressById } from 'apiSdk/addresses';
import { AddressInterface } from 'interfaces/address';

type ColumnType = ColumnDef<AddressInterface, unknown>;
...
export function AddressListPage(props: AddressListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AddressInterface>>(
    () => `/addresses?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AddressInterface) => {
    if (hasAccess('address', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/addresses/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'street', header: 'Street', accessorKey: 'street' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/addresses/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('address', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/addresses/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('address', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="address" />
        </Flex>

        {hasAccess('address', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/addresses/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
