...
import { getPhones, deletePhoneById } from 'apiSdk/phones';
import { PhoneInterface } from 'interfaces/phone';

type ColumnType = ColumnDef<PhoneInterface, unknown>;
...
export function PhoneListPage(props: PhoneListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getPhones({
        ...
        order: params.order,
        searchTermKeys: ['number.contains', 'type.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PhoneInterface>>(
    () => `/phones?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PhoneInterface) => {
    if (hasAccess('phone', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/phones/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/phones/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('phone', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/phones/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('phone', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="phone" />
        </Flex>

        {hasAccess('phone', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/phones/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
